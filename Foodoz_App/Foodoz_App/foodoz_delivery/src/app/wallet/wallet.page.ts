...
export class WalletPage implements OnInit, OnDestroy {
  ...
  handleTransactionRes(res: BaseListResponse) {
    this.transactions = this.transactions.concat(this.spliceFor(res.data, ["withdraw"]));
  ...
  private spliceFor(pgs: Array<WalletTransaction>, removeFor: Array<string>): Array<WalletTransaction> {
    let indexToRemove = -1;
    for (let i = 0; i < pgs.length; i++) {
      if (removeFor.includes(pgs[i].type)) {
        indexToRemove = i;
        break;
      }
    }
