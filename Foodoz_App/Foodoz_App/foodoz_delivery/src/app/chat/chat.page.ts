...
export class ChatPage implements OnInit {
  ...
  constructor(private apiService: ApiService, private router: Router, private callNumber: CallNumber,
    private uiElementService: UiElementsService, private translate: TranslateService, private navCtrl: NavController) { }

  ngOnInit() {
    if (this.router.getCurrentNavigation().extras.state && this.router.getCurrentNavigation().extras.state.chat)
      this.chatObj = this.router.getCurrentNavigation().extras.state.chat;
    else
      this.navCtrl.pop();
    //this.chatObj = this.navParams.get("chat");
   if(this.router.getCurrentNavigation().extras.state && this.router.getCurrentNavigation().extras.state.dialing_number) this.dialingNumber = this.router.getCurrentNavigation().extras.state.dialing_number;
  ...
  dialNo() {
    console.log(this.dialingNumber);
    if (this.dialingNumber) {
      this.callNumber.callNumber(this.dialingNumber, false).then(res => console.log('Launched dialer!', res)).catch(err => console.log('Error launching dialer', err));
    } else {
