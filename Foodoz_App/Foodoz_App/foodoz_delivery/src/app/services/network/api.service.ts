...
export class ApiService {
  ...
  public getSampleOrder(): Observable<Order> {
    return this.http.get<Order>('./assets/json/sample_order.json').pipe(tap(data => this.setupOrder(data)));
  }
  ...
  public setupOrder(order: Order) {
    ...
    for (let product of order.products) {
      ...
      if (product.vendor_product) {
        ...
        if (product.vendor_product.price) product.vendor_product.priceToShow = this.currency_icon + Number(product.vendor_product.price).toFixed(2);
        if (product.vendor_product.sale_price) product.vendor_product.salePriceToShow = this.currency_icon + Number(product.vendor_product.sale_price).toFixed(2);
        ...
        if (!product.addon_choices) product.addon_choices = [];
        for (let addOnChoice of product.addon_choices) addOnChoice.addon_choice.priceToShow = this.currency_icon + Number(addOnChoice.addon_choice.price).toFixed(2);
