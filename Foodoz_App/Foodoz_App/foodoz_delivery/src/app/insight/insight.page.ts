...
export class InsightPage implements OnInit, OnDestroy {
  ...
  setupDefaultInsightsOrder() {
    ...
    if (this.insightsOrder && this.insightsOrder.orders_chart_data.length) {
      ...
      for (let i = 0; i <= 5; i++) this.priceRangeOrder.push(Number((((i == 0) ? maxEarning : (this.priceRangeOrder[i - 1] - breaker))).toFixed()));
      this.chartDataParsedOrder = [];
      for (let cd of this.insightsOrder.orders_chart_data) this.chartDataParsedOrder.push({ date: this.getDefoultValue(cd.period, insightRequest), percent: String(((Number(cd.total) * 100) / maxEarning)), price: "" });
    }
  }
  ...
  setupDefaultInsightsEarning() {
    ...
    if (this.insightsEarning && this.insightsEarning.earnings_chart_data.length) {
      ...
      for (let cd of this.insightsEarning.earnings_chart_data) this.chartDataParsedEarning.push({ date: this.getDefoultValue(cd.period, insightRequest), percent: String(((Number(cd.total) * 100) / maxEarning)), price: "" });
    }
  }
    ...
    this.translate.get("loading").subscribe(value => {
      ...
      this.subscriptions.push(this.apiService.getInsightEarning(insightRequest).subscribe(res => {
        ...
        if (this.insightsEarning && this.insightsEarning.earnings_chart_data.length) {
          ...
          let breaker = maxEarning / 5;
          this.priceRangeEarning = [];
          for (let i = 0; i <= 5; i++) this.priceRangeEarning.push(Number((((i == 0) ? maxEarning : ((this.priceRangeEarning[i - 1] - breaker) > 0 ? (this.priceRangeEarning[i - 1] - breaker) : 0))).toFixed()));
          this.chartDataParsedEarning = [];
          for (let cd of this.insightsEarning.earnings_chart_data) this.chartDataParsedEarning.push({ date: this.getPeriodText(cd.period, insightRequest), percent: String(this.adjustPercent(((Number(cd.total) * 100) / maxEarning))), price: this.currency_icon + " " + Number(cd.total).toFixed() });
        }
        else {
          this.setupDefaultInsightsEarning();
        }
  ...
  loadInsightOrders() {
    ...
    this.subscriptions.push(this.apiService.getInsightOrder(Helper.getProfile().id, insightRequest).subscribe(res => {
      ...
      if (this.insightsOrder && this.insightsOrder.orders_chart_data.length) {
        ...
        let breaker = maxEarning / 5;
        this.priceRangeOrder = [];
        for (let i = 0; i <= 5; i++) this.priceRangeOrder.push(Number((((i == 0) ? maxEarning : ((this.priceRangeOrder[i - 1] - breaker) > 0 ? (this.priceRangeOrder[i - 1] - breaker) : 0))).toFixed()));
        this.chartDataParsedOrder = [];
        for (let cd of this.insightsOrder.orders_chart_data) this.chartDataParsedOrder.push({ date: this.getPeriodText(cd.period, insightRequest), percent: String(this.adjustPercent(((Number(cd.total) * 100) / maxEarning))), price: Number(cd.total).toFixed() });
      }
      else {
        this.setupDefaultInsightsOrder();
      }
      ...
      this.uiElementService.dismissLoading();
      console.log("chartDataParsedOrder", this.chartDataParsedOrder);
    }, err => {
      console.log("insights", err);
      this.uiElementService.dismissLoading();
    }));
  ...
  private adjustPercent(percent: number): string {
    return String(percent < 10 ? 10 : Math.round(percent));
  }
