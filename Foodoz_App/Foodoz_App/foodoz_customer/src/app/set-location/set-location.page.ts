...
export class SetLocationPage implements OnInit {
  ...
  save() {
    if (this.location.formatted_address && this.location.formatted_address.length > 2) {
  ...
  onAddressTypeChange(event) {
    if (event.detail && event.detail.value) {
      this.location.title = event.detail.value;
    }
  }
  ...
  alertLocationServices() {
    this.translate.get(['location_services_title', 'location_services_message', "no", 'okay']).subscribe(text => {
