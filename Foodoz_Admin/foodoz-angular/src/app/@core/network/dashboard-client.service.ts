...
export class DashboardClient extends BaseClient {
  ...
  public activeOrders(): Observable<Array<Order>> {
    return this.authService.getToken().pipe(switchMap((token) => {
      return this.http.get<Array<Order>>(this.getBaseEndpoint() + '/active-order-locations',
        {headers: this.getHeaders(token)});
    }));
  }

  public activeDelivery(): Observable<Array<Delivery>> {
    return this.authService.getToken().pipe(switchMap((token) => {
      return this.http.get<Array<Delivery>>(this.getBaseEndpoint() + '/active-delivery',
        {headers: this.getHeaders(token)});
    }));
  }
